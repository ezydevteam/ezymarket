...
<head>
    ...
    <title>{{ translate('Admin Product Statistics Report') }} - {{ $product->name }}</title>
    <style>
        @page {
            margin: 1cm;
        }
...
<body>
    <div class="report-wrapper">
                ...
                <tr>
                    <td class="header-cell">
                        ...
                        <div class="product-meta">
                            #{{ $product->id }} | {{ translate('Seller') }}: {{ $product->seller->full_name}} | {{ $product->category->name ?? 'N/A' }}
                        </div>
                    </td>
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card">
                <div class="metric-label">{{ translate('Sales Amount') }}</div>
            ...
            <div class="metric-card">
                <div class="metric-label">{{ translate('Total Comments') }}</div>
                <div class="metric-value">{{ number_format($counters['total_comments']) }}</div>
            </div>
        </div>
        ...
        <div class="section">
            ...
            @if($topCountries->count() > 0)
                <table class="data-table">
                    ...
                    <tbody>
                        @php $totalSales = $topCountries->sum('total_sales'); @endphp
                        @foreach($topCountries as $country)
                            <tr>
                                ...
                                <td class="text-right">{{ $totalSales > 0 ? number_format(($country->total_sales / $totalSales) * 100, 1) : 0 }}%</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="section">
            ...
            @if($referrals->count() > 0)
                <table class="data-table">
                    ...
                    <tbody>
                        @php $totalViewsCount = $referrals->sum('total_views'); @endphp
                        @foreach($referrals as $referral)
                            <tr>
                                <td>{{ $referral->referrer ?: translate('Direct / Unknown') }}</td>
                                <td class="text-center">{{ number_format($referral->total_views) }}</td>
                                <td class="text-right">{{ $totalViewsCount > 0 ? number_format(($referral->total_views / $totalViewsCount) * 100, 1) : 0 }}%</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                        ...
                        @foreach($sales as $sale)
                            <tr>
                                ...
                                <td class="row-highlight">
                                    {{ $sale->user->username ?? translate('Guest') }}
                                    @if($sale->user && $sale->user->email)
                                        <div style="font-size: 9px; color: #64748b; font-weight: 400;">{{ $sale->user->email }}</div>
                                    @endif
                                </td>
                                <td class="text-right">{{ getAmount($sale->price) }}</td>
                                <td class="text-right" style="color: #166534; font-weight: 600;">{{ getAmount($sale->seller_earning + (@$sale->seller_tax->amount ?? 0)) }}</td>
    ...
    <div class="footer">
        {{ translate('Generated by') }} {{ settings('general')->site_name }} — {{ translate('Admin Statistics Report') }}
    </div>
