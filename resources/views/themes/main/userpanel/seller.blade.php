...
@section('title', translate('Become a Seller'))

@section('content')
    <div class="card-v rounded-4 shadow-sm p-4">
        <div class="mb-5">
            <div class="col-lg-10 m-auto mb-5">
                <div class="text-center">
                    <h1 class="mb-4 fw-bold text-gradient">{{ translate('Become a Seller') }}</h1>
                    <p class="text-gray-700">
                        {{ translate('Unleash your creativity and earn by joining our platform as an Seller! As an Seller, you have the unique opportunity to craft and sell your own products, reaching a wide audience eager to discover new content. Whether you are a developer, designer, or you have any unique products') }}
                    </p>
                </div>
            </div>
            <div class="col-12 col-xxl-8 m-auto">
                <div class="row g-4">
                    <div class="{{ @$settings->referral->status ? 'col-lg-6' : 'col-12' }}">
                        <div class="card bg-light border rounded-4 p-4 text-center h-100">
                            <div class="mb-4">
                            ...
                            <h4>{{ translate('Upload and sell your products') }}</h4>
                            <p class="text-gray-700 fs-14">
                                {{ translate('Start selling your products today! Our easy upload process allows you to quickly bring your unique products to a global audience.') }}
                            </p>
                        </div>
                    </div>
        ...
        <div class="text-center">
            <form action="{{ route('user.become_seller.store') }}" class="ajax-form" method="POST">
                @csrf
                @if (@$settings->links->seller_terms_link)
                    <div class="mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input border-secondary me-2" type="checkbox" name="seller_terms" id="seller-terms"
                                {{ old('seller_terms') ? 'checked' : '' }} required>
                            <label class="form-check-label" for="seller-terms">
                                {{ translate('I read and I agree with the') }}
                                <a href="{{ @$settings->links->seller_terms_link }}">
                                    {{ translate('Product seller terms.') }}
                                </a>
                            </label>
                        </div>
                    </div>
                ...
                @if (@$settings->referral->status && @$settings->links->referral_terms_link)
                    <div class="mb-3">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input border-secondary me-2" type="checkbox" name="referral_terms"
                ...
                @endif
                <button type="submit"
                    class="btn btn-primary btn-modern btn-lg rounded-pill fw-bold px-5 ls-1 mt-4 action-confirm">
