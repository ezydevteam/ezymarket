<div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview-tab">
    ...
    <div class="card mb-4">
        <div class="card-header bg-white p-3">
            <h4 class="mb-0">{{ translate('2. Get All products') }}</h4>
        </div>
        <div class="card-body p-4">
            <p><strong>{{ translate('Endpoint') }}:</strong>
                <code>{{ translate('GET') }} {{ route('api.products.all') }}</code>
            </p>
            <p class="mb-0">
                <strong>{{ translate('Description') }}:</strong>
                {{ translate('Retrieves all products associated with the provided API key.') }}
            </p>
        </div>
    </div>
    <div class="card mb-4">
        <div class="card-header bg-white p-3">
            <h4 class="mb-0">{{ translate('3. Get An product Details') }}</h4>
        </div>
        <div class="card-body p-4">
            <p><strong>{{ translate('Endpoint') }}:</strong>
                <code>{{ translate('GET') }} {{ route('api.products.product') }}</code>
            </p>
            <p class="mb-0">
                <strong>{{ translate('Description') }}:</strong>
