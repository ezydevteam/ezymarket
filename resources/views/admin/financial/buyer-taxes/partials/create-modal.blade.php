@php
    $modalId = 'createBuyerTaxModal';
    $formId = 'createBuyerTaxForm';
@endphp

<x-modal
    id="{{ $modalId }}"
    :title="translate('New Buyer Tax')"
    size="md"
    icon="bi-bag-plus"
>
    <form id="{{ $formId }}" action="{{ route('admin.financial.buyer-taxes.store') }}" method="POST">
        @csrf
    ...
    <x-slot name="footer">
        ...
        <button type="submit" form="{{ $formId }}" class="btn btn-primary ms-3" id="createBuyerTaxBtn">
