@php
    $taxId = $tax->id;
    $modalId = "editSellerTaxModal-{$taxId}";
    $formId = "editSellerTaxForm-{$taxId}";
@endphp

<x-modal
    id="{{ $modalId }}"
    :title="translate('Edit Seller Tax')"
    size="md"
    icon="bi-pencil-square"
>
    <form id="{{ $formId }}" action="{{ route('admin.financial.seller-taxes.update', $taxId) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="col-12">
                ...
                <div class="input-group">
                    <input type="number" name="percentage" class="form-control form-control-md"
                        value="{{ $tax->percentage }}" placeholder="0.00" min="0.01" max="100" step="0.01" required />
    ...
    <x-slot name="footer">
        ...
        <button type="submit" form="{{ $formId }}" class="btn btn-primary ms-3" id="editSellerTaxBtn-{{ $taxId }}">
