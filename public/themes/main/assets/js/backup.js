(function($) {
	...
	function initializeDropzoneUpload(retryCount = 0) {
		...
		function addUploadedFileToList(response) {
			...
			const thumbnail = imageTypes.includes(response.mime_type)
				...
				: `<span class="bi bi-file-earmark-zip fs-2 text-primary" data-type="${response.extension}"></span>`;
				...
				<div class="uploaded-file uploaded-file-${response.id}">
					...
					<div class="uploaded-file-info">
						<h6 class="uploaded-file-name">
							<span class="success-mark"><i class="bi bi-check-circle"></i></span>
		...
		function handleDiscountPercentage($percentageInput, updateFunction) {
			$percentageInput.on('input', function() {
				...
				if (percentage > maxDiscountPercentage) {
					...
					const config = getProductConfig();
					alert(config?.translates.max_discount_percentage_error || 'Maximum discount percentage is not correct');
	...
	function initializeSubmissionFeatures() {
		...
		// Handle Form Submission Styling
		$form.on('submit', function() {
			const $submitBtn = $('#productSubmitBtn');
			$submitBtn.html(`<span class="spinner-border spinner-border-sm text-white me-2" role="status" aria-hidden="true"></span>Submitting...`)
					  .prop('disabled', true);
		});
	...
	function saveDraft($form) {
		...
		const config = getProductConfig();
		const draftUrl = config?.save_draft_route ?? '/user/product/save_draft';
		...
		$.ajax({
			...
			success: function(result) {
				if (result.success) {
					// Update hidden draft_id field if returned
					if (result.data && result.data.product_id) {
						$('input[name="draft_id"]').val(result.data.product_id);
					}
	...
	function collectFormData($form) {
		...
		$form.find('select[name*="options["]').each(function() {
			...
			if (match) {
				...
				data.options[optionId] = $select.prop('multiple') ? (val || []) : (val || '');
			}
		});

		// CKEditor
		if (window.productDescriptionEditor) data['description'] = window.productDescriptionEditor.getData();
		else data['description'] = $('#productSubmitDescription').val();

		// Tags
		const $tagsInput = $('#product-tags');
		if ($tagsInput.length) {
			try {
				data['tags'] = $tagsInput.tagsinput('items');
			} catch(e) {
				data['tags'] = $tagsInput.val();
			}
		}

		// Switches / Boolean fields (explicitly set to 0/1)
		data['has_custom_services'] = $('#customServicesSwitch').is(':checked') ? 1 : 0;
		data['custom_services'] = $('#custom_services').val().trim();
		data['free_product'] = $('#free_product_check').is(':checked') ? 1 : 0;
		data['purchasing_status'] = $('#purchasing_status_check').is(':checked') ? 1 : 0;

		const isAdditionalFeatures = $('#additionalFeatureSwitch').is(':checked') ? 1 : 0;
		if (isAdditionalFeatures === 1) {
			data['regular_extra_features'] = convertTextareaToArray($('#regular_extra_features').val());
			data['extended_extra_features'] = convertTextareaToArray($('#extended_extra_features').val());
		}
		...
		$categorySelect.on('change', function() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					// Update Buyer Fees in config
					config.buyer_fee.regular = response.file_config.buyer_fee.regular;
					config.buyer_fee.extended = response.file_config.buyer_fee.extended;

					// Update Upload Config
					config.upload = response.file_config.upload;

					// Update File loading route
