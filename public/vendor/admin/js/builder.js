    ...
    class BuilderManager {
        ...
        populateSectionSettings(options) {
            ...
            $.each(options, function (key, value) {
                ...
                } else if (key === 'is_full_width') {
                    const isChecked = value === '1' || value === 1 || value === true;
                    form.find('#is_full_width').prop('checked', isChecked).trigger('change');
            ...
            $(document).on('submit', '#editBlockForm, #editHomeBlockForm', function (e) {
                ...
                self.processFormWithUploads($form, function (finalData) {
                    ...
                    if (uniqueId) {
                        ...
                        // Handle all other checkboxes — unchecked ones are not submitted by HTML forms
                        $form.find('input[type="checkbox"].form-check-input').each(function () {
                            const name = $(this).attr('name');
                            if (name && name !== 'is_active' && name !== 'block_status') {
                                finalData[name] = $(this).is(':checked') ? '1' : '0';
                            }
                        });
                        ...
                        if (typeof finalData.subtitle !== 'undefined') {
                            $item.attr('title', finalData.subtitle);
                        }
                    }
        ...
        updateRowLayout(row, colsType) {
            ...
            else if (colsType == '2') newColsDef = [6, 6]; // 50% + 50%
            else if (colsType == '3') newColsDef = [4, 4, 4]; // 33% + 33% + 33%
        ...
        bindSectionSettingsUI() {
            ...
            // Reusable: data-hide-toggle="#target" — hides target when checked, shows when unchecked
            this.handleToggleHide($('#sectionSettingsForm'));
        ...
        /**
         * Reusable: Init & bind [data-hide-toggle] within a container.
         * Checkbox checked → hide target; unchecked → show target.
         * Works on dynamically loaded content (delegated).
         */
        handleToggleHide($container) {
            const toggle = function ($el, animate = true) {
                const target = $el.data('hide-toggle');
                const $target = $(target);
                const isChecked = $el.is(':checked');
                if (isChecked) {
                    if (animate) $target.slideUp(200); else $target.hide();
                } else {
                    $target.removeClass('d-none');
                    if (animate) $target.slideDown(200); else $target.show();
                }
            };

            // Delegated change event
            $container.off('change', '[data-hide-toggle]').on('change', '[data-hide-toggle]', function () {
                toggle($(this), true);
            });

            // Set initial state without animation
            $container.find('[data-hide-toggle]').each(function () {
                toggle($(this), false);
            });
        }
        ...
        initEditBlockUI() {
            ...
            // Reusable: data-hide-toggle
            this.handleToggleHide($(document.body));
            ...
            if (window.Coloris) {
                const offcanvasBody = document.querySelector('#editBlockOffcanvas .offcanvas-body')
                    || document.querySelector('#editHomeSectionOffcanvas .offcanvas-body');
                const parentSelector = offcanvasBody
                    ? (offcanvasBody.closest('#editBlockOffcanvas') ? '#editBlockOffcanvas .offcanvas-body' : '#editHomeSectionOffcanvas .offcanvas-body')
                    : null;
                Coloris({
        ...
        processFormWithUploads($form, successCallback, errorCallback) {
            ...
            chain.then(() => {
                ...
                const contentKeyOrder = []; // Track content keys in DOM order
                for (let [key, value] of formData.entries()) {
                    if (value instanceof File) continue;
                    setValue(serializedData, key, value);

                    // Capture content key order from FormData (follows DOM order)
                    const contentMatch = key.match(/^content\[([^\]]+)\]/);
                    if (contentMatch) {
                        const contentKey = contentMatch[1];
                        if (!contentKeyOrder.includes(contentKey)) {
                            contentKeyOrder.push(contentKey);
                        }
                    }
                }
                uploadedFiles.forEach(f => {
                    ...
                    // Also track uploaded file content keys
                    const contentMatch = f.name.match(/^content\[([^\]]+)\]/);
                    if (contentMatch) {
                        const contentKey = contentMatch[1];
                        if (!contentKeyOrder.includes(contentKey)) {
                            contentKeyOrder.push(contentKey);
                ...
                if (serializedData.content && typeof serializedData.content === 'object' && !Array.isArray(serializedData.content)) {
                    // Use DOM-order keys (from FormData iteration) to preserve drag-sort order
                    // Object.values() sorts numeric keys ascending, which loses reordering
                    if (contentKeyOrder.length > 0) {
                        serializedData.content = contentKeyOrder.map(key => serializedData.content[key]).filter(Boolean);
                    } else {
