...
namespace Codebay\Chunk\Handler;

use Illuminate\Http\Request;
use Illuminate\Http\UploadedFile;
use Session;
use Codebay\Chunk\Config\AbstractConfig;
use Codebay\Chunk\Save\AbstractSave;
use Codebay\Chunk\Storage\ChunkStorage;
...
abstract class AbstractHandler
{
    ...
    public function createChunkFileName($additionalName = null, $currentChunkIndex = null)
    {
        ...
        $array = [
            $this->changeChunkFileName($this->file),
        ];
    ...
    /**
     * Change the filename of an uploaded file to a unique hash value.
     *
     * @param \Illuminate\Http\UploadedFile $file The uploaded file to rename
     *
     * @return string The modified filename
     */
    public function changeChunkFileName($file)
    {
        // Get the file extension and original filename
        ...
        if (empty($fileNoExt) || empty($fileExtension)) {
            // If the filename or extension is empty, use the hash of the filename
            $filename = md5($fileNoExt);
        } else {
            // Otherwise, use the hash of the filename with the extension appended
            $filename = md5($fileNoExt) . '.' . strtolower($fileExtension);
        }
